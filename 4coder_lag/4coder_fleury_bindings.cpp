...
//#include "4coder_lag.cpp"

bool isLowercase(u8 v)
{
    return (v >= 'a' && v <= 'z');
}

bool isUppercase(u8 v)
{
    return (v >= 'A' && v <= 'Z');
}
...
bool isText(u8 v)
{
    return isLowercase(v) || isUppercase(v) || v == '_';
}

bool getCharAtCursor(Application_Links* app, Buffer_ID buffer, View_ID view, i64 relativePos, u8& val)
{
    i64 start = view_get_cursor_pos(app, view);
    Range_i64 r;
    r.start = start + relativePos;
    r.end = start + relativePos + 1;
    if (r.start <= 0)
    {
        return false;
    }
...
CUSTOM_COMMAND_SIG(lag_case_move_left)
CUSTOM_DOC("Moves curser to previous word in the casing.")
{
    ...
    // Setup
    u8 val = 0;
    CaseType type = CASETYPE_WHITESPACE;
    if (getCharAtCursor(app, buffer, view, 0, val))
    {
        if (isLowercase(val))
        {
            if (getCharAtCursor(app, buffer, view, -1, val))
            {
                if (!isLowercase(val) && !isUppercase(val))
                {
                    type = CASETYPE_WHITESPACE;
                }
                else
                {
                    type = CASETYPE_LOWER;
        ...
        else if (getCharAtCursor(app, buffer, view, -1, val))
        {
            if (isLowercase(val))
            {
                type = CASETYPE_LOWER;
            ...
            {
                if (getCharAtCursor(app, buffer, view, -2, val))
                {
                    if (!isUppercase(val) || val == '_')
                    {
                        move_left(app);
                        return;
                    }
    ...
    while (getCharAtCursor(app, buffer, view, 0, val))
    {
        if (isLowercase(val))
        {
            if (type == CASETYPE_UPPER)
            {
                type = CASETYPE_LOWER;
            }
            else if (type == CASETYPE_WHITESPACE)
            {
                move_right(app);
                break;
            }
...
CUSTOM_COMMAND_SIG(lag_case_move_right)
CUSTOM_DOC("Moves curser to next word in the casing.")
{
    View_ID view = get_active_view(app, 0);
    Buffer_ID buffer = view_get_buffer(app, view, 0);
    u8 tBuffer[256];
...
CUSTOM_COMMAND_SIG(lag_case_backspace)
CUSTOM_DOC("Backspace a single word in the casing.")
{
    View_ID view = get_active_view(app, 0);
    Buffer_ID buffer = view_get_buffer(app, view, 0);
    
    i64 beginPos = view_get_cursor_pos(app, view);
    lag_case_move_left(app);
    i64 endPos = view_get_cursor_pos(app, view);
    buffer_replace_range(app, buffer, Ii64(beginPos, endPos), string_u8_empty);
}

CUSTOM_COMMAND_SIG(lag_switch_matching_file_cpp)
...
CUSTOM_COMMAND_SIG(lag_init_keys)
CUSTOM_DOC("Initialize bindings.")
{
    //Thread_Context* tctx = get_thread_context(app);
    //mapping_init(tctx, &framework_mapping);
    //MappingScope();
    //SelectMapping(&framework_mapping);
    //Bind(lag_case_move_left, KeyCode_Left, KeyCode_Alt, KeyCode_Control);
...
Fleury4SetBindings(Mapping *mapping)
{
    ...
    //Bind(lag_case_move_left, KeyCode_F12);
    ...
    // NOTE(rjf): Custom bindings.
    {
        ...
        Bind(lag_case_move_left, KeyCode_Left, KeyCode_Alt);
        Bind(lag_case_move_right, KeyCode_Right, KeyCode_Alt);
        Bind(lag_case_backspace, KeyCode_Backspace, KeyCode_Alt);
        Bind(lag_switch_matching_file_cpp, KeyCode_K, KeyCode_Alt);
        Bind(lag_goto_next_error, KeyCode_Right, KeyCode_Shift);
        Bind(lag_goto_previous_error, KeyCode_Left, KeyCode_Shift);
        Bind(lag_build_search, KeyCode_B, KeyCode_Control);
        
        Bind(lag_autocomplete, KeyCode_ForwardSlash, KeyCode_Control);
        Bind(lag_autocomplete_move_down, KeyCode_Down, KeyCode_Shift);
        Bind(lag_autocomplete_move_up, KeyCode_Up, KeyCode_Shift);
        Bind(lag_autocomplete_write, KeyCode_Return, KeyCode_Shift);
        Bind(lag_autocomplete_close, KeyCode_Escape, KeyCode_Shift);
    }
    ...
    Bind(open_matching_file_cpp,     KeyCode_2, KeyCode_Alt); //<<<<<<< SWITCH CPP, MODIFY THIS
