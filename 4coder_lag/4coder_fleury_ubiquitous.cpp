...
Fleury4DrawCTokenColors(Application_Links *app, Text_Layout_ID text_layout_id, Token_Array *array)
{
    ...
    for(;;)
    {
        ...
        if(token->kind == TokenBaseKind_Identifier && token_it_inc_all(&it))
        {
            ...
            {
                u8 tBuffer[256];
                if (token->size < 256 && buffer_read_range(app, buffer, Ii64_size(token->pos, token->size), tBuffer))
                {
                    String_Const_u8 rawStr;
                    rawStr.str = tBuffer;
                    rawStr.size = token->size;
                    if (string_compare(rawStr, string_u8_litexpr("u8")) == 0 ||
                        string_compare(rawStr, string_u8_litexpr("u16")) == 0 ||
                        string_compare(rawStr, string_u8_litexpr("u32")) == 0 ||
                        string_compare(rawStr, string_u8_litexpr("u64")) == 0 ||
                        string_compare(rawStr, string_u8_litexpr("i8")) == 0 ||
                        string_compare(rawStr, string_u8_litexpr("i16")) == 0 ||
                        string_compare(rawStr, string_u8_litexpr("i32")) == 0 ||
                        string_compare(rawStr, string_u8_litexpr("i64")) == 0 ||
                        string_compare(rawStr, string_u8_litexpr("f32")) == 0 ||
                        string_compare(rawStr, string_u8_litexpr("f64")) == 0 ||
                        string_compare(rawStr, string_u8_litexpr("v2")) == 0 ||
                        string_compare(rawStr, string_u8_litexpr("v3")) == 0 ||
                        string_compare(rawStr, string_u8_litexpr("v4")) == 0 ||
                        string_compare(rawStr, string_u8_litexpr("fp")) == 0)
                    {
                        argb = 0xffff3300; // fcolor_resolve(fcolor_id(defcolor_pop2));
                    }
                    else if (isType(rawStr))
                    {
                        argb = 0xff0066ff;
                    }
                }
