                    ...
                    Rect_f32 rect, Frame_Info frame_info)
{
    ...
    {
        lagRenderAutocomplete(app, active_view, global_small_code_face, buffer, frame_info);
...
Fleury4Render(Application_Links *app, Frame_Info frame_info, View_ID view_id)
{
    ...
    // NOTE(LAG): Draw errors
    {
        Buffer_ID cViewBuffer = view_get_buffer(app, active_view, 0);
        Face_ID faceId = get_face_id(app, cViewBuffer);
        Face_Metrics fm = get_face_metrics(app, faceId);
        //View_Context viewCon = view_current_context(app, active_view);
        Rect_f32 screenRect = region;//view_get_screen_rect(app, active_view);
        Buffer_Scroll buffScroll = view_get_buffer_scroll(app, active_view);
        f32 scrollY = (buffScroll.position.line_number - 1) * fm.line_height + buffScroll.position.pixel_shift.y;
        for (i64 i = 0; i < errorsCount; ++i)
        {
            if (errors[i].buffer == cViewBuffer)
            {
                Rect_f32 erRect;
                erRect.x0 = screenRect.x0;
                erRect.x1 = screenRect.x1 - 5.f;
                erRect.y0 = fm.line_height * (f32)errors[i].lineNum - scrollY;
                erRect.y1 = erRect.y0 + fm.line_height;
                draw_rectangle(app, erRect, 0.f, 0xffffffff);
            }
        }
    }
...
BUFFER_HOOK_SIG(Fleury4BeginBuffer)
{
    ...
    *eol_setting = setting;
    ...
    /*// Find and store type information for highlighting.
    Token_Array tokenArray = get_token_array_from_buffer(app, buffer_id);
    for (i64 i = 0; i < tokenArray.count; ++i)
    {
        Token& it = tokenArray.tokens[i];
        //if ()
    }
    */
    
    // Types
    //findTypes(app, buffer_id);
